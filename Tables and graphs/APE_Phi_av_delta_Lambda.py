...
def calculate_ape():
    ...
    model and a direct numerical solution for the Lambda parameter space.
    """
    # Adjust these variables to match the exact names of your input files
    file_analytic = "PhiAv_vs_delta_varying_Lambda.txt"
    file_direct = "Direct_PhiAv_vs_delta_varying_Lambda.txt"
    file_output = "APE_PhiAv_vs_delta_varying_Lambda.txt"
    ...
    try:
        ...
        print(f"Ensure '{file_analytic}' and '{file_direct}' exist in the current directory.")
        return

    # Retrieve headers from the reference (analytic) file
    headers = lines_a[0].strip('\n').split('\t')
    ...
    with open(file_output, 'w') as f_out:
        ...
        for la, ld in zip(lines_a[1:], lines_d[1:]):
            ...
            if not parts_a or not parts_d or parts_a[0] == '':
                continue
            ...
            # Iterate through the numerical columns sequentially
            for val_a_str, val_d_str in zip(parts_a[1:], parts_d[1:]):
                # Convert to mpmath.mpf for multi-precision evaluation
                ...
                # Format APE to 5 significant digits using scientific notation (4 decimal places)
