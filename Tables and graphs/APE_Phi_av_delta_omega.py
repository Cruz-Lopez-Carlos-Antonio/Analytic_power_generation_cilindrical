...
def calculate_ape():
    ...
    model and a direct numerical solution for the omega parameter space.
    """
    # Adjust these variables if your input files have different names
    file_analytic = "PhiAv_vs_delta_varying_omega.txt"
    file_direct = "Direct_PhiAv_vs_delta_varying_omega.txt"
    file_output = "APE_PhiAv_vs_delta_varying_omega.txt"
    ...
    try:
        ...
        print(f"Ensure that '{file_analytic}' and '{file_direct}' exist in the current directory.")
        return
    ...
    with open(file_output, 'w') as f_out:
        ...
        for la, ld in zip(lines_a[1:], lines_d[1:]):
            ...
            # The first element is the spatial or index parameter 'delta'
            delta = parts_a[0]
            ape_row = [delta]

            # Iterate through the numerical columns dynamically
