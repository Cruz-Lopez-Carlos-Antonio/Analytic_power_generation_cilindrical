...
Module[{localDir = DirectoryName[$InputFileName]},
  ...
  Get[FileNameJoin[{localDir, "PoissonBoltzmann.wl"}]];
];
...
omegaFIntegrand[R_?NumericQ] := F[R] Sinh[PsiPB[R]] R;
...
omegaFAPE = If[Abs[Omega] > 0, 100 omegaFAbsoluteError/Abs[Omega], Indeterminate];
...
integratedResidualF[R_?NumericQ] :=
  MR[R] R FPrime[R] - (PiD R^2/2 + Omega Lambda delta^2 HR[R]);

residualGrid = N[Subdivide[0, 1, nResidualPoints - 1], 30];
residualValues = integratedResidualF /@ residualGrid;
residualLInf = Max[Abs[residualValues]];
residualL2 = Sqrt[Mean[residualValues^2]];
